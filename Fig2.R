...
# This script shows how to reproduce the results in Figure 2 of Gupta et. al (version June 2020)
...
# Supplementary Table 3. Identification of best accuracy thresholds for fold change in prevalence and difference in prevalence between Healthy and Nonhealthy
baseline[baseline < 0.001] <- 0
Healthy <- baseline[,grep('Healthy', names(baseline))]
Nonhealthy <- baseline[,-grep('Healthy', names(baseline))]
PH<-apply(Healthy, 1, function(i) (sum(i > 0))*100/2636) # species prevalence among healthy
PNH<-apply(Nonhealthy, 1, function(i) (sum(i > 0))*100/1711) # species prevalence among non-healthy
PH_diff<-(PH-PNH)
PH_fold<-(PH/PNH)
PNH_fold<-(PNH/PH)
...
for(i in 1:length(cutoffs1)) {
  ...
  for(j in 1:length(cutoffs2)) {
    H_signature_sublist[[j]] <- data.frame(subset(as.matrix(all_matrix), all_matrix$PH_fold >= cutoffs1[i] & all_matrix$PH_diff >= cutoffs2[j]))
    NH_signature_sublist[[j]] <- data.frame(subset(as.matrix(all_matrix), all_matrix$PNH_fold >= cutoffs1[i] & all_matrix$PH_diff <= -cutoffs2[j]))
    ...
    report_sublist[[j]]<-cbind(cutoffs1[i],cutoffs2[j],nrow(H_signature_sublist[[j]]),nrow(NH_signature_sublist[[j]]),Healthy_accuracy,Nonhealthy_accuracy,total_accuracy,total_average_accuracy)
    report[[i]] <- report_sublist
  }
}
Accuracy_table <- matrix(unlist(report), ncol = 8, byrow = TRUE)
colnames(Accuracy_table) <- c("Fold change", "Difference in coverage b/w healthy and non-healthy","H+ count","H- count","Healthy accuracy","Non-healthy accuracy","Total accuracy","Total average accuracy")
Supplementary_Table3<-data.frame(Accuracy_table)
Supplementary_Table3<-Supplementary_Table3[,c(1:4,8)]
colnames(Supplementary_Table3)<-c("Fold change", "Difference in prevalence b/w healthy and Nonhealthy","Health-prevalent species count","Health-scarce species count","Classification accuracy")
...
colnames(final_dataset1)[c(2,3,4,6:11)]<-c("study1","age","BMI","Phenotype","FBG","TRIG","LDLC","CHOL","HDLC")
...
table2_input<-data.frame(cbind(PH,PNH,PH_diff,PH_fold,PNH_fold))
...
colnames(table2)<-c("Prevalence in Healthy samples (%)","Prevalence in Nohealthy samples (%)","Difference (PH-PNH)","Fold-change (PH/PNH or PNH/PH)")
...
HDLC_plot +rremove("legend")+theme(axis.text=element_text(size=14,face="bold"),axis.title=element_text(size=14,face="bold"))+scale_colour_manual(values=c("GMHI_pos"="steelblue","GMHI_neg"="orange2"))+stat_compare_means(label = "p.format",method = "wilcox.test",label.x.npc = "middle")+scale_fill_manual(values=c("GMHI_pos"="steelblue","GMHI_neg"="orange2"))+labs(x = "",y="High-Density Lipoproteins Cholesterol (mg/dL)")
